...
<head>
    ...
    <title>Verification Logs - LOGAREA</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <script defer src="https://cdn.jsdelivr.net/npm/alpinejs@3.x.x/dist/cdn.min.js"></script>
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&display=swap" rel="stylesheet">
    <style> body { font-family: 'Inter', sans-serif; } [x-cloak] { display: none !important; } </style>
</head>
<body class="bg-gray-50 text-gray-800" x-data="{ expanded: true, modalOpen: false, selectedLogId: null, actionType: 'approved' }">
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <main class="flex-1 overflow-y-auto p-8">
            <header class="flex justify-between items-center mb-10">
                <h1 class="text-2xl font-bold text-black">Verification Logs</h1>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden text-black">
                <table class="w-full text-left text-[12px]">
                    <thead class="bg-indigo-50/50 text-gray-600 font-bold border-b border-gray-100 uppercase tracking-wider">
                        <tr>
                            <th class="px-4 py-4 text-black">Staff Name</th>
                            <th class="px-4 py-4 text-black">Log Date</th>
                            <th class="px-4 py-4 w-1/3 text-black">Activity Summary</th>
                            <th class="px-4 py-4 text-black">Status</th>
                            <th class="px-4 py-4 text-center text-black">Action</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-50">
                        @forelse($logs as $log)
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-4 py-4 font-semibold text-black">{{ $log->user->user_name }}</td>
                            <td class="px-4 py-4 text-black">{{ $log->log_date->format('Y-m-d') }}</td>
                            <td class="px-4 py-4 text-black leading-relaxed">{{ $log->activity_summary }}</td>
                            <td class="px-4 py-4">
                                <span class="px-2 py-0.5 rounded {{ $log->status == 'pending' ? 'bg-yellow-100 text-yellow-700' : ($log->status == 'approved' ? 'bg-[#E6E1FF] text-[#5B3AFF]' : 'bg-red-100 text-red-700') }}">
                                    {{ ucfirst($log->status) }}
                                </span>
                            </td>
                            <td class="px-4 py-4 text-center">
                                @if($log->status == 'pending')
                                    <button @click="selectedLogId = {{ $log->id_logs }}; modalOpen = true" class="bg-[#5B3AFF] text-white px-3 py-1 rounded text-[10px] hover:bg-indigo-600 transition">Verify</button>
                                @else 
                        ...
                        @empty
                        <tr><td colspan="5" class="px-4 py-10 text-center text-gray-400 italic">No logs found for verification.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
    ...
    <div x-show="modalOpen" x-cloak class="fixed inset-0 bg-black/50 flex items-center justify-center z-[60]" x-transition.opacity>
        <div class="bg-white rounded-xl w-full max-w-md p-6" @click.away="modalOpen = false">
            <h3 class="text-lg font-bold mb-4 text-black">Process Verification</h3>
            <form :action="'/verification/' + selectedLogId" method="POST" class="space-y-4 text-sm text-black">
                @csrf
                <div>
                    <label class="block mb-1 font-semibold">Decision</label>
                    <select name="action" x-model="actionType" class="w-full border border-gray-300 rounded-lg p-2 focus:ring-2 focus:ring-[#5B3AFF] outline-none">
                        <option value="approved">Approve Activity</option>
                        <option value="rejected">Reject Activity</option>
                    </select>
                </div>
                <div>
                    <label class="block mb-1 font-semibold">Note / Feedback</label>
                    <textarea name="verification_note" rows="3" class="w-full border border-gray-300 rounded-lg p-2 focus:ring-2 focus:ring-[#5B3AFF] outline-none" placeholder="Required if you reject this activity..."></textarea>
                </div>
                <div class="flex justify-end gap-2 pt-2">
                    <button type="button" @click="modalOpen = false" class="px-4 py-2 text-gray-500 hover:bg-gray-100 rounded-lg transition">Cancel</button>
                    <button type="submit" class="bg-[#5B3AFF] text-white px-5 py-2 rounded-lg font-medium hover:bg-indigo-600 transition shadow-sm">Submit Decision</button>
                </div>
            </form>
        </div>
    </div>
